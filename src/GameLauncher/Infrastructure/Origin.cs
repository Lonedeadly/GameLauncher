...
public static class Origin
{
    ...
    ///
    /// Перебор только на отказе соединения — в том числе на молчании: у
    /// клиента короткий ConnectTimeout, и порт, который не отвечает, тоже
    /// даёт HttpRequestException. Ответ 404 — это ответ: сервер на связи и
    /// говорит, что такого файла нет. Пробовать из-за него другие порты
    /// значило бы получить те же три 404 и втрое дольше.
    ///
    /// Отмена снаружи — это не отказ порта, а конец запроса: её не ловим.</summary>
